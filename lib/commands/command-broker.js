...
(function() {
  ...
  module.exports = {
    ...
    listenForCommand: function(commandName, data, cb, finishCmdName) {
      var listenCb, result;
      this._checkSocket();
      result = Q.defer();
      ...
      listenCb = (function(_this) {
        return function(data) {
          ...
          if (finishCmdName === command.name || finishCmdName === command.id) {
            console.log("finish cmd received. finish");
            _this.socket.removeListener('data', listenCb);
            return result.resolve(command.data);
      ...
      this.socket.once('data', (function(_this) {
        return function(data) {
          ...
          if (commandName === command.name || commandName === command.id) {
            ...
            console.log(command.data);
            return deferred.reject(command.data);
          } else if (command.id === 'helo') {
            console.log("kDHello received. Newton is alive and connected");
            return _this.receiveCommand(commandName).then(function(result) {
